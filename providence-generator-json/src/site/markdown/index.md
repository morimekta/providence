...
The model is identical to the reflection model used (see
`providence-reflect/src/main/providence/model.thrift` for reference). Each
containing the `ThriftDocument` model parsed from the thrift file. It uses
the `JsonSerializer` from the `providence-core` module with named fields and
enums.
