 ...
 * @since 19.09.15
 */
public abstract class TMessageBodyReader<T extends PMessage<T>> implements MessageBodyReader<T> {
    private final PSerializer mSerializer;

    public TMessageBodyReader(PSerializer serializer) {
        mSerializer = serializer;
    }

    @SuppressWarnings("unchecked")
    private PStructDescriptor<T,?> getDescriptor(Class<?> type) {
        try {
            if (!PMessage.class.isAssignableFrom(type))
                return null;
            Method method = type.getMethod("provider");
            PStructDescriptorProvider<T,?> provider = (PStructDescriptorProvider<T,?>) method.invoke(null);
    ...
    public T readFrom(Class<T> type,
        ...
        try {
            PStructDescriptor<T,?> descriptor = getDescriptor(type);
